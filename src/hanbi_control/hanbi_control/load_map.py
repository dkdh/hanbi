...
class loadMap(Node):
    ...
    def __init__(self):
        ...
        self.map_offset_x=-50.0
        self.map_offset_y=-50.0
        ...
        full_path = "C:\\dev\\ros2_smart_home\\src\\hanbi_control\\map\\map1005.txt"
        print(full_path)
        ...
        for num,data in enumerate(line):
            line_data=list(map(int,data.split()))
            for idx, data in enumerate(line_data):
                self.map_data[num] = data

        print(self.map_meta_data)
