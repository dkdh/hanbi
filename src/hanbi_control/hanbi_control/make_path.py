...
class makePath(Node):
    ...
    def __init__(self):
        ...
        # self.turtle_sub = self.create_subscription(TurtlebotStatus, '/turtlebot_status', self.listener_callback,10)

        '''
        로직 2. 저장할 경로 및 텍스트파일 이름을 정하고, 쓰기 모드로 열기
        '''

        full_path= "C:\\dev\\ros2_smart_home\\src\\hanbi_control\\path\\path_odom_kjh.txt"
    ...
    def listener_callback(self,msg):
        # print('x : {} , y : {} '.format(msg.twist.angular.x,msg.twist.angular.y))
        if self.is_odom ==False :   
            ...
            '''
            distance = sqrt(pow(y-self.prev_y,2) + pow(x - self.prev_x,2))
                
            # 로직 5. 거리차이가 위치보다 0.1m 이상일 때 위치를 path_msg.poses에 추가하고 publish
            if distance > 0.1 :
                waypoint_pose.pose.position.x=float(x)
                waypoint_pose.pose.position.y=float(y)
