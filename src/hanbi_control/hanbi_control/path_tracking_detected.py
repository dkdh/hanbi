...
class followTheCarrot(Node):
    ...
    def __init__(self):
        ...
        self.path_sub = self.create_subscription(Path,'/detected_local_path',self.path_callback,10)
        self.detected_end = self.create_publisher(String, 'detected_end', 10)
    ...
    def timer_callback(self):
        ...
        if self.is_status and self.is_odom ==True and self.is_path==True:
            ...
            if len(self.path_msg.poses)> 1:
                ...
                detected_end_msg = String()
                detected_end_msg.data = self.path_msg.header.frame_id
                self.detected_end.publish(detected_end_msg)
                self.path_msg = Path()
                self.is_path=False
            ...
            print("status : {0} / odom : {1} / path : {2}".format(self.is_status, self.is_odom, self.is_path))
