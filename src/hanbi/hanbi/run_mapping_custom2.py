...
class Mapping:
    ...
    def __init__(self, params_map):
        ...
        self.map = np.ones((self.map_size[0].astype(np.int32), self.map_size[1].astype(np.int32)))*0.5
        is_map_exist = True
    ...
    def update(self, pose, laser):
        ...
        for i in range(laser_global.shape[1]):
            ...
            for y, x in zip(avail_y, avail_x):
                # if(self.map[y,x] >=1): continue
                self.map[y,x] += self.occu_down
                buf.append((int(y), int(x), self.map[y, x]))
    ...
    def show_pose_and_points(self, pose, laser_global):
        ...
        for i in range(laser_global.shape[1]):
            (l_x, l_y) = np.array([laser_global_x[i], laser_global_y[i]]).astype(np.int64)
            center = (l_x, l_y)
...
def main(args=None):    
    ...
    def connect():
        ...
        # if(is_map_exist==True):
        #     print("yes")
        #     for i in range(5):
        #         buf = [i] + run_mapping.mapping.map[i]
        #         sio.emit("Map2ServerInit", run_mapping.mapping.map)
        # else:
        #     print("no")
        #     for i in range(5):
        #         buf = [i] + [0 for x in range(500)]
        #         sio.emit("Map2ServerInit", buf)
