...
class Mapping:
    ...
    def update(self, pose, laser):
        ...
        for i in range(laser_global.shape[1]):
            ...
            # finalY, finalX = avail_y[-1], avail_x[-1]
            # self.map[finalY, finalX] = self.map[finalY, finalX] - self.occu_up
            # sio.emit("Map2Server", [int(finalY), int(finalX),self.pixel(finalY,finalX)])
    ...
    def __del__(self):
        ...
        # self.save_map(())
        full_path = "C:\\dev\\ros2_smart_home\\src\\hanbi_control\\map\\map_final5.txt"
        open(full_path, "w")
