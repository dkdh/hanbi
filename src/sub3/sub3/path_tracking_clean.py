...
class followTheCarrot(Node):
        ...
        if self.is_status and self.is_odom ==True and self.is_path==True and self.is_lidar==True:
            ...
            if self.is_a_star == True and self.a_star_flag == True:
                ...
                if len(self.a_star_msg.poses) > 1:
                    ...
                    if self.thread_flag_2 == False:
                        ...
                        # 쓰레기 버리기 로직일 경우
                        thread_pick = threading.Thread(target=self.hand_control_put_down)               
                        thread_pick.start()
            ...
            if len(self.path_msg.poses)> 1:
                ...
                if self.is_look_forward_point :
                    ...
                    if self.collision == True:
                        if self.hanvi_msg.detections:
                            ...
                            # 분실물
                                ...
                                if self.thread_flag_3 == False:
                                    self.thread_flag_3 = True
                                    thread_pick = threading.Thread(target=self.lost_pick_up)               
                                    thread_pick.start()
                    ...
                    # 물체를 드는 순간 라이다가 인식을 못하고 self.collision이 False로 바뀐다.
    ...
    def hand_control_pick_up(self):
        ...
        # self.goal_pose_msg.pose.position.x = -7.14
        # self.goal_pose_msg.pose.position.y = 7.12
        self.goal_pub.publish(self.goal_pose_msg)
        # 돌아올 곳
